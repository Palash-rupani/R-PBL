...
ui <- dashboardPage(
  ...
  dashboardSidebar(
    ...
    selectInput("sel_year", "📅 Year Range:",
                choices  = c("All", 2001:2014),
                selected = "All"),
    selectInput("sel_district", "📍 District:",
                choices  = c("All", sort(unique(
                  ap_crime$DISTRICT[!str_detect(ap_crime$DISTRICT, "TOTAL|ZZ|RLY|RAILWAY")]
                ))),
                selected = "All"),
    br(),
  ...
  dashboardBody(
    ...
    tabItems(
      ...
      tabItem(tabName = "overview",
              ...
              fluidRow(
                ...
                box(
                  ...
                  withSpinner(plotlyOutput("plot_trend", height = 300), color = "#e94560")
                ),
                # Risk distribution
                box(
                  title = "🎯 Risk Level Distribution",
                  width = 4, status = "primary", solidHeader = TRUE,
                  withSpinner(plotlyOutput("plot_risk_pie", height = 300), color = "#0f3460")
                )
              ),
              ...
              fluidRow(
                # Top districts
                box(
                  title = "🏆 Top 10 Most Crime-Prone Districts",
                  width = 6, status = "warning", solidHeader = TRUE,
                  withSpinner(plotlyOutput("plot_top_districts", height = 320), color = "#f5a623")
                ),
                # Crime categories
                box(
                  title = "📊 Crime Categories Breakdown",
                  width = 6, status = "success", solidHeader = TRUE,
                  withSpinner(plotlyOutput("plot_categories", height = 320), color = "#27ae60")
                )
              )
      ),
      ...
      tabItem(tabName = "map",
              fluidRow(
                box(
                  ...
                  column(3, selectInput("map_type", "Map Type:",
                                        choices = c(
                                          "Crime Intensity"  = "intensity",
                                          "Violent Crimes"   = "violent",
                                          "Property Crimes"  = "property",
                                          "Women Crimes"     = "women"
                                        ))),
                  column(3, selectInput("map_year_filter", "Year:",
                                        choices  = c("All", 2001:2014),
                                        selected = "All")),
                  column(6, tags$div(style = "padding-top:25px; color:#666; font-size:13px;",
                                     "💡 Click any circle for district details. Bubble size = crime volume."))
                )
              ),
              fluidRow(
                box(
                  title = "🗺️ Interactive Crime Map — AP / Telangana",
                  width = 12, status = "danger", solidHeader = TRUE,
                  withSpinner(leafletOutput("crime_map", height = 550), color = "#e94560")
                )
              )
      ),
      ...
      tabItem(tabName = "viz",
              fluidRow(
                box(
                  title = "📈 Crime Trend — AP/Telangana",
                  width = 6, status = "danger", solidHeader = TRUE,
                  withSpinner(plotlyOutput("viz_trend", height = 300), color = "#e94560")
                ),
                box(
                  title = "🏙️ Hyderabad vs Rest of AP",
                  width = 6, status = "primary", solidHeader = TRUE,
                  withSpinner(plotlyOutput("viz_hyd_vs_ap", height = 300), color = "#0f3460")
                )
              ),
              fluidRow(
                box(
                  ...
                  withSpinner(plotlyOutput("viz_heatmap", height = 380), color = "#f5a623")
                )
              ),
              fluidRow(
                box(
                  title = "👩 Women Safety Trends",
                  width = 6, status = "success", solidHeader = TRUE,
                  withSpinner(plotlyOutput("viz_women", height = 300), color = "#27ae60")
                ),
                box(
                  title = "🔍 Crime Type Breakdown — Hyderabad",
                  width = 6, status = "danger", solidHeader = TRUE,
                  withSpinner(plotlyOutput("viz_breakdown", height = 300), color = "#e94560")
                )
              )
      ),
      ...
      tabItem(tabName = "ml",
              fluidRow(
                box(
                  title = "🤖 Crime Risk Predictor — Powered by Random Forest (82.6% Accuracy)",
                  width = 12, status = "danger", solidHeader = TRUE,
                  tags$p(style = "color:#666; margin-bottom:15px;",
                         "Adjust the sliders to match a district's crime stats.
                    The ML model will instantly predict its risk level.")
                )
              ),
              fluidRow(
                # Input sliders
                box(
                  title = "⚙️ Input Crime Statistics", width = 5,
                  status = "primary", solidHeader = TRUE,
                  sliderInput("sl_murder",   "🔪 Murder cases:",    0, 300, 50, step = 5),
                  sliderInput("sl_rape",     "⚠️ Rape cases:",      0, 300, 80, step = 5),
                  sliderInput("sl_theft",    "🏠 Theft cases:",     0, 5000, 1000, step = 50),
                  sliderInput("sl_burglary", "🚪 Burglary cases:",  0, 2000, 300, step = 25),
                  sliderInput("sl_riots",    "⚡ Riot cases:",      0, 500, 100, step = 10),
                  sliderInput("sl_violent",  "💢 Violent crimes:",  0, 2000, 400, step = 25),
                  sliderInput("sl_property", "📦 Property crimes:", 0, 5000, 1200, step = 50),
                  sliderInput("sl_women",    "👩 Women crimes:",    0, 1000, 200, step = 10),
                  sliderInput("sl_year",     "📅 Year:",            2001, 2014, 2010, step = 1),
                  checkboxInput("sl_hyd",    "📍 Is Hyderabad region?", value = FALSE),
                  br(),
                  actionButton("btn_predict", "🔮 Predict Risk Level",
                               class = "btn btn-danger btn-lg btn-block")
                ),
                
                # Output
                box(
                  title = "🎯 Prediction Result", width = 7,
                  status = "warning", solidHeader = TRUE,
                  uiOutput("ml_result"),
                  br(),
                  withSpinner(plotlyOutput("ml_prob_plot", height = 280), color = "#e94560"),
                  br(),
                  tags$div(style = "color:#888; font-size:12px; padding:10px;",
                           "⚡ Model: Random Forest | Trees: 500 | Features: 22",
                           br(),
                           "📊 Training accuracy: 82.6% | AUC (HIGH): 0.989")
                )
              )
      ),
      ...
      tabItem(tabName = "cyber",
              ...
              fluidRow(
                box(
                  title = "💻 Cybercrime Cases by State (2018)",
                  width = 7, status = "danger", solidHeader = TRUE,
                  withSpinner(plotlyOutput("cyber_bar", height = 380), color = "#e94560")
                ),
                box(
                  title = "📈 AP + Telangana Cybercrime Trend (2016–2018)",
                  width = 5, status = "primary", solidHeader = TRUE,
                  withSpinner(plotlyOutput("cyber_trend", height = 380), color = "#0f3460")
                )
              ),
              fluidRow(
                box(
                  title = "📊 Cybercrime Rate per Lakh Population",
                  width = 12, status = "warning", solidHeader = TRUE,
                  withSpinner(plotlyOutput("cyber_rate", height = 300), color = "#f5a623")
                )
              )
      ),
      ...
      tabItem(tabName = "data",
              fluidRow(
                box(
                  ...
                  column(3, selectInput("tbl_risk", "Risk Level:",
                                        ...
                                        selected = "All")),
                  column(3, tags$div(style = "padding-top:25px;",
              ...
              fluidRow(
                box(
                  title = "📋 Full Crime Dataset — AP / Telangana",
                  width = 12, status = "primary", solidHeader = TRUE,
...
server <- function(input, output, session) {
  ...
  output$box_worst <- renderValueBox({
    top <- filtered() %>%
      ...
      group_by(DISTRICT) %>%
      summarise(T = sum(TOTAL.IPC.CRIMES, na.rm = TRUE)) %>%
  ...
  output$plot_trend <- renderPlotly({
    ...
    plot_ly(df, x = ~YEAR, y = ~TOTAL, type = "scatter", mode = "lines+markers",
  ...
  output$plot_top_districts <- renderPlotly({
    df <- filtered() %>%
      group_by(DISTRICT) %>%
      summarise(AVG = mean(TOTAL.IPC.CRIMES, na.rm = TRUE)) %>%
  ...
  output$plot_categories <- renderPlotly({
    ...
    plot_ly(df, x = ~YEAR) %>%
      add_bars(y = ~Violent,  name = "Violent",  marker = list(color = "#e94560")) %>%
      add_bars(y = ~Property, name = "Property", marker = list(color = "#0f3460")) %>%
      add_bars(y = ~Women,    name = "Women",    marker = list(color = "#f5a623")) %>%
  ...
  output$crime_map <- renderLeaflet({
    ...
    map_col <- switch(input$map_type,
                      "intensity" = "TOTAL_CRIMES",
    ...
    leaflet(df) %>%
      ...
      addCircleMarkers(
        ...
        popup = ~paste0(
          ...
          "<h3 style='color:#e94560; margin:0 0 6px 0'>", DISTRICT, "</h3>",
  ...
  output$viz_hyd_vs_ap <- renderPlotly({
    ...
    df <- ap_crime %>%
      filter(!str_detect(DISTRICT, "TOTAL|ZZ")) %>%
      mutate(REGION = ifelse(DISTRICT %in% hyd_d, "Hyderabad", "Rest of AP")) %>%
  ...
  output$viz_heatmap <- renderPlotly({
    ...
    df <- ap_crime %>%
      filter(DISTRICT %in% key_districts) %>%
      group_by(DISTRICT, YEAR) %>%
      summarise(TOTAL = sum(TOTAL.IPC.CRIMES, na.rm = TRUE), .groups = "drop")
    mat <- tidyr::pivot_wider(df, names_from = YEAR, values_from = TOTAL, values_fill = 0)
    z   <- as.matrix(mat[,-1])
    plot_ly(x = colnames(z), y = mat$DISTRICT, z = z,
            type = "heatmap",
            colorscale = list(c(0,"#f8f9fa"), c(0.5,"#fc9272"), c(1,"#67000d"))) %>%
      layout(xaxis = list(title = "Year"),
             yaxis = list(title = ""),
             plot_bgcolor  = "#ffffff",
             paper_bgcolor = "#ffffff") %>%
  ...
  output$viz_women <- renderPlotly({
    ...
    plot_ly(df, x = ~Year) %>%
      add_lines(y = ~Rape,       name = "Rape",       line = list(color = "#e94560")) %>%
      add_lines(y = ~Kidnapping, name = "Kidnapping", line = list(color = "#0f3460")) %>%
      add_lines(y = ~Dowry,      name = "Dowry",      line = list(color = "#f5a623")) %>%
      add_lines(y = ~Cruelty,    name = "Cruelty",    line = list(color = "#27ae60")) %>%
      layout(xaxis = list(title = "Year"),
  ...
  prediction <- eventReactive(input$btn_predict, {
    new_data <- data.frame(
      MURDER                              = input$sl_murder,
      RAPE                                = input$sl_rape,
      KIDNAPPING...ABDUCTION              = 0,
      ROBBERY                             = 0,
      BURGLARY                            = input$sl_burglary,
      THEFT                               = input$sl_theft,
      AUTO.THEFT                          = 0,
      RIOTS                               = input$sl_riots,
      CHEATING                            = 0,
      ARSON                               = 0,
      DOWRY.DEATHS                        = 0,
      CRUELTY.BY.HUSBAND.OR.HIS.RELATIVES = 0,
      CAUSING.DEATH.BY.NEGLIGENCE         = 0,
      VIOLENT_CRIMES                      = input$sl_violent,
      PROPERTY_CRIMES                     = input$sl_property,
      WOMEN_CRIMES                        = input$sl_women,
      VIOLENT_RATIO  = ifelse(input$sl_violent + input$sl_property > 0,
                              input$sl_violent / (input$sl_violent + input$sl_property), 0),
      PROPERTY_RATIO = ifelse(input$sl_violent + input$sl_property > 0,
                              input$sl_property / (input$sl_violent + input$sl_property), 0),
      WOMEN_RATIO    = 0.1,
  ...
  output$ml_result <- renderUI({
    ...
    emoji <- switch(pred,
                    ...
                    "LOW"    = "🟢")
    tags$div(
      style = paste0("text-align:center; padding:20px; background:", color,
                     "15; border-radius:10px; border: 2px solid ", color),
      tags$h2(style = paste0("color:", color, "; margin:0; font-size:36px;"),
              paste(emoji, pred, "RISK")),
      tags$p(style = "color:#666; margin:8px 0 0 0; font-size:14px;",
             "Based on Random Forest model with 82.6% accuracy")
    )
  })
  ...
  output$ml_prob_plot <- renderPlotly({
    ...
    plot_ly(df, x = ~Level, y = ~Prob, type = "bar",
      ...
      layout(
        ...
        yaxis  = list(title = "Probability (%)", range = c(0, 110)),
        plot_bgcolor  = "#ffffff",
        paper_bgcolor = "#ffffff"
      ) %>%
      config(displayModeBar = FALSE)
  ...
  output$cyber_trend <- renderPlotly({
    df <- cyber_clean %>%
      ...
      tidyr::pivot_longer(-STATE, names_to = "Year", values_to = "Cases") %>%
      mutate(Year = as.integer(str_extract(Year, "\\d+")))
    plot_ly(df, x = ~Year, y = ~Cases, color = ~STATE,
            type = "scatter", mode = "lines+markers",
            colors = c("ANDHRA PRADESH" = "#0f3460", "TELANGANA" = "#e94560"),
  ...
  table_data <- reactive({
    ...
    if (input$tbl_state != "All") df <- df %>% filter(STATE.UT  == input$tbl_state)
    if (input$tbl_year  != "All") df <- df %>% filter(YEAR      == as.integer(input$tbl_year))
    if (input$tbl_risk  != "All") df <- df %>% filter(RISK_LEVEL == input$tbl_risk)
    df
  })
  ...
  output$crime_table <- renderDT({
    datatable(
      ...
      formatStyle("TOTAL.IPC.CRIMES",
                  background      = styleColorBar(range(ap_crime$TOTAL.IPC.CRIMES), "#0f3460"),
